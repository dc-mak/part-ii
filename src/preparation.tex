...
\prechapter{%
  ...
  which aim to help a user understand a Coq library and a description of Neo4j
...
modify the code, propogate the necessary changes, evaluate the end-result and
...
\href{https://travis-ci.org}{Travis-CI} for automated continous-integration
builds.
...
First, it is almost always wiser to work with and modify existing sytems (e.g.\
...
aformentioned \emph{Feit-Thompson} theorem (approximately 170,000 lines, 15,000
...
Coq is a \emph{huge} project, devloped by INRIA (France), and its size and
...
analyse their strengths and weakeness. What follows is a detailed account of
...
flexibility~\ref{req:m2} and size/precomputation~\ref{req:m3}) this approach was
abandoned.
...
add the missing pieces to the model and output it as to comma-separated values
...
this project by being easy to understand, flexibile to use and extensible; it
...
not well-suited to more intense analyses over large graphs of libaries and is
...
are described in Subsesction~\ref{subsec:alg}

Although igraph is not as easy to interact with  as APOC, the extra capabilities
afforded were indispensible towards achieving the \emph{computation}
requirements of a core library of good defaults.

\subsection{igraph Algorithms}
...
language}, part of the Free Software Foundation's GNU poject. It is relevant for
...
difficult system to work with. I also compared existing tools which aim to help
a user understand a Coq library (coqdoc, coqdep, CoqSerAPI, dpdgraph) in
relation to my project's requirements and showed that although none satisified
